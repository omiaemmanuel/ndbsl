...
export default function AdminLayout() {
  ...
  const NAV_ITEMS = [
    ...
    { to: '/admin/procurement', label: t('nav.requests') },
    { to: '/admin/equipment', label: t('nav.equipment') },
    { to: '/admin/bookings', label: t('nav.bookings') },
    { to: '/admin/consumables', label: t('nav.consumables') },
    { to: '/admin/faults', label: t('nav.faults') },
    { to: '/admin/research', label: t('nav.research') },
    { to: '/admin/announcements', label: t('nav.announcements') },
    { to: '/admin/queries', label: t('nav.queries') },
    { to: '/admin/expenditure', label: t('nav.expenditure') },
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="p-5 border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{t('portal.admin')}</p>
            </div>
          </div>
        </div>
      ...
      <HelpButton role={user?.role === 'super_admin' ? 'super_admin' : 'admin'} />
    </div>
  );
}
